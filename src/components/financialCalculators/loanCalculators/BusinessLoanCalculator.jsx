...
const BusinessLoanCalculator = () => (
  <CalculatorShell
    ...
    subtitle="Calculate your business loan EMI and repayment summary."
    icon="ph:briefcase"
    fields={[
      { name: 'loanAmount', label: 'Loan Amount', type: 'number', prefix: '₹', placeholder: '0' },
      { name: 'rateOfInterest', label: 'Rate of Interest (P.A.)', type: 'number', suffix: '%', placeholder: '0' },
      { name: 'loanTenure', label: 'Loan Tenure', type: 'number', suffix: 'years', placeholder: '0' },
    ]}
    compute={(v) => {
      const P = parseFloat(v.loanAmount) || 0;
      const annualRate = parseFloat(v.rateOfInterest) || 0;
