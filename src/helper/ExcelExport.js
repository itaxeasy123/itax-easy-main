...
const handleExport = async (data, fileName = "Data") => {
    const workbook = new ExcelJS.Workbook();

    const getMaxWidth = (sheet, columnKey) =>
        Math.max(
            columnKey.length,
            ...data[sheet].bodyData.map(item =>
                item[columnKey]?.toString().length || 0
            )
        );

    Object.entries(data).forEach(([sheetName, { headers, bodyData, bannerData = [], specialHeader }]) => {
        const worksheet = workbook.addWorksheet(sheetName);

        // Columns
        worksheet.columns = Object.entries(headers).map(([header, key]) => ({
            header,
            key,
            width: getMaxWidth(sheetName, key) + 2,
        }));

        // Data
        bodyData.forEach(row => worksheet.addRow(row));

        // Header style
        const headerRow = worksheet.getRow(1);
        headerRow.height = 40;
        headerRow.eachCell(cell => {
            createCellStyle(cell, {
        ...
        worksheet.eachRow((row, rowNum) => {
            if (rowNum > 1 && rowNum % 2 !== 0) {
                row.eachCell(cell => createCellStyle(cell, { fillColor: "FFbfdbfe", border: false }));
            } else {
                row.eachCell(cell => createCellStyle(cell, { border: false }));
            }
        });

        // Banner rows
        bannerData.forEach(banner => {
            worksheet.insertRow(banner.startRow, []);
            worksheet.mergeCells(banner.startRow, banner.startCol, banner.endRow, worksheet.columns.length);
            const cell = worksheet.getRow(banner.startRow).getCell(1);
            cell.value = banner.hyperlink
                ? { text: banner.text, hyperlink: banner.hyperlink, tooltip: banner.hyperlinkText }
                : banner.text;
            createCellStyle(cell, {
                font: {
                    size: banner.fontSize,
                    bold: banner.fontBold,
                    color: { argb: banner.textColor },
                    italic: banner.fontItalic,
                    underline: banner.fontUnderline,
                },
                fillColor: banner.backgroundColor,
            });
            if (banner.height) worksheet.getRow(banner.startRow).height = banner.height;
        });

        // Special header
        if (specialHeader) {
            const rowValues = [];
            specialHeader.forEach(el => {
                rowValues[el.startCol] = el.text;
                worksheet.mergeCells(el.startRow, el.startCol, el.endRow, el.endCol);
            });
            const insertedRow = worksheet.insertRow(4, rowValues);
            insertedRow.height = 40;
            insertedRow.eachCell(cell =>
